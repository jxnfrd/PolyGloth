...
| `lib/ui/explain.ts` | The plain-English layer: glossary (hover definitions), human labels for every signal type, `describeSignal()` → one sentence per signal, and the what/how/what-to-do help text shown under every page title. Change wording here, not in pages. |
