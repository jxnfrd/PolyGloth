...
import { Stat, Table, H, Pill, MarketLink, fmtUsd, fmtPct, fmtTs, ago, Signed, Empty, short, fmtDec } from '@/components/intel/ui';
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-6">
                ...
                <Stat label="Calibrated ROI" value={allTime ? fmtPct(allTime.calibrated_roi) : '—'} tone={allTime && Number(allTime.calibrated_roi) > 0 ? 'pos' : allTime ? 'neg' : 'muted'} sub={allTime?.p_value != null ? `p = ${Number(allTime.p_value).toFixed(3)}` : undefined} />
            ...
            <Table head={['category', 'window', 'resolved', 'win %', 'staked', 'P/L', 'ROI', 'calibrated ROI', 'p-value', 'Brier', 'timing (pp)']}
            ...
            <Table head={['when', 'type', 'score', 'market', 'outcome']} rows={signals.map(s => [ago(s.ts), <Pill key="t" tone="sky">{String(s.type)}</Pill>, s.score == null ? '—' : Number(s.score).toFixed(0), <MarketLink key="m" conditionId={s.condition_id} />, s.outcome ? <Pill key="o" tone={s.outcome === 'WIN' ? 'emerald' : 'rose'}>{String(s.outcome)}</Pill> : '—'])} />
